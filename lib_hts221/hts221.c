...
void HTS221_citaj(uint8_t adresa_registra, uint8_t* hodnoty, size_t dlzka) {
    i2c_master_read(hodnoty, dlzka, adresa_registra, adresa_hts221, 1);
}

// Write multiple bytes
void HTS221_zapisuj(uint8_t adresa_registra, uint8_t* hodnoty, size_t dlzka) {
    i2c_master_write_multi(hodnoty, dlzka, adresa_registra, adresa_hts221, 0);
}

void HTS221_init(void) {
    uint8_t zistenie_senzoru;

    HTS221_read_bytes(HTS221_WHO_AM_I_ADDRESS, &zistenie_senzoru, 1);
